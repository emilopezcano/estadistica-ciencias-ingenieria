...
distris <- data.frame(
  Distribución = c("$\\text{Bernoulli}\\\\ \\mathit{Ber}(p)$",
                   "$\\text{Binomial}\\\\ \\mathit{Bin}(n;p)$",
                   "$\\text{Geométrica}\\\\ \\mathit{Ge}(p)$",
                   "$\\text{Binomial negativa}\\\\ \\mathit{BN}(r;p)$",
                   "$\\text{Poisson}\\\\ \\mathit{Poiss}(\\lambda)$",
                   "$\\text{Hipergeométrica}\\\\ \\mathit{HG}(N; M; N)$"),
  Probabilidad = c("$X = \\begin{cases} 1 & \\mbox{ con probabilidad } p \\\\ 0 & \\mbox{ con probabilidad } 1-p \\end{cases}$",
                   "$P[X = x] = \\binom{n}{x}\\cdot p^x \\cdot (1-p)^{(n-x)};\\\\ x = 0, 1, \\ldots, n$",
                   "$P[X = x] = p \\cdot (1-p)^{x};\\\\ x = 0, 1, \\ldots, \\infty$",
                   "$P[X = x] =\\binom{x+c-1}{x}\\cdot p^c \\cdot (1-p)^{x};\\\\ x = 0, 1, 2, \\ldots, \\infty$",
                   "$P[X = x] = \\frac{e^{-\\lambda}\\lambda^x}{x!};\\\\ x = 0, 1, \\ldots\ \\infty$",
                   "$P[X = x] = \\frac{\\binom{N-M}{n-x}\\cdot \\binom{M}{x}}{\\binom{N}{n}};\\\\ \\max{(0, n+M-N)} \\leq x \\leq \\min{(M,n)}$"),
  Esperanza = c("$p$",
                "$n\\cdot p$",
                "$\\frac{1-p}{p}$",
                "$\\frac{r \\cdot (1-p)}{ p}$",
...
```{r, echo=FALSE}
distris <- data.frame(
  Distribución = c("$\\text{Uniforme}\\\\ \\mathit{U}(a;b)$",
                   "$\\text{Exponencial}\\\\ \\mathit{Exp}(\\beta)$",
                   "$\\text{Normal}\\\\ \\mathit{N}(\\mu; \\sigma)$"),
  Probabilidad = c("$f(x) = 
\\begin{cases}
...
\\end{cases} \\\\ a<x<b$",
                   "$f(x) = 
\\beta e^{-\\beta x},\\; x > 0\\\\F(x)=\\int_{-\\infty}^xf(t)dt=1-e^{-\\beta x}, \\; x > 0$",
                   "$f(x) = 
