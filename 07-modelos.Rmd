...
En el ejemplo de los sujetos en estudio, 
...
```{r plotbinom, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Representación de la función de probabilidad del modelo binomial", out.width="70%", message=FALSE}
library(dplyr)
library(ggplot2)
n <- c(5, 10, 20)
...
Es decir, en promedio el primer investigador será el número 13 (ya que 12 es el número medio de no investigadores)
```
...
$\{X_n\}, n \in \mathbb{N}$ cumple Teorema central del límite si y solo si:
...
Condición suficiente: que las variables de la sucesión sean **independientes e idénticamente distribuidas**
...
```{r, tclbinom, echo=FALSE,fig.width=6, fig.asp=1.2, fig.align='center', fig.cap="El teorema central del límite y la distribución binomial", out.width="70%"}
par(mfrow=c(2,2))
for (n in c(10, 30, 50, 100)){
  ...
  plot(dbinom(x, n, p), 
       type = "n", 
       xlim = c(-1, n+1), axes = FALSE, main = paste0("Binomial p=", p, ", n=", n),
       xlab = "x", ylab = "P[X=x]")
  axis(1, pos = 0, at = seq(-1, n+1), labels= c(NA, seq(0, n+1)))
  axis(2, las = 2, pos = -1)
  for (i in x){
    lines(rep(i, 2), c(0, dbinom(i, n, p)), lwd=3, col = "steelblue")
  }
}
```
