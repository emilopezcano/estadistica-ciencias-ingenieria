...
mediante técnicas de combinatoria, véase al apéndice \@ref(ap:combinatoria).
...
situación habitual descrita en la figura \@ref(fig:dogma) al principio de este
...
6. $P(\bigcup\limits_{i=1}^n A_i) = \sum\limits_{i=1}^n P(A_i) - \sum\limits_{i<j} P(A_i \cap A_j) + \sum\limits_{i<j<k} P(A_i \cap A_j \cap A_k) - \ldots + (-1)^{n-1} P \left(\bigcap\limits_{i=1}^n A_i\right )$.
...
$$A: \text{Al menos dos personas de un grupo de 30 cumplen años el mismo día,}$$

entonces el suceso complementario es:

$$A^c: \text{No hay dos personas en un grupo de 30 que cumplen años el mismo día.}$$
...
las variaciones utilizando los factoriales (ver apéndice \@ref(ap:combinatoria)), 
