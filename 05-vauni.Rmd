...
```{r dogma2b, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Variables aleatorias vs. datos empíricos", out.width="70%"}
library(grid)
...
grid.lines(c(0.8, 0.8, 0.4, 0.4), 
           ...
           arrow = arrow(length = unit(0.15, "inches")))
grid.text(expression(bold(italic(X))), 0.65, 0.65, gp = gpar(cex = 4))
grid.text(expression(mu), 0.3, 0.3, gp = gpar(cex = 5))
grid.text(expression(sigma), 0.45, 0.35, gp = gpar(cex = 5))
grid.text(expression(italic(bar(x))), 0.85, 0.42, gp = gpar(cex = 2))
grid.text(expression(italic(S)), 0.9, 0.45, gp = gpar(cex = 2))
vp.dens <- viewport(x = 0.35, y = 0.6,
                    height = 0.25, width = 0.3,
                    name = "dens")
pushViewport(vp.dens)
# grid.rect(gp = gpar(col="#CCCCCC"))
set.seed(66)
p <- ggplot(data.frame(x = c(30, 36)), aes(x)) + 
  # geom_histogram(bins = 7, fill = "steelblue", col = "white") +
  stat_function(fun = dnorm, args = list(mean = 33, sd = 1), size=1.5) +
  theme_void()
  # labs(x = "Volumen", y = "f(x)")
print(p, newpage = FALSE)
popViewport()
vp.hist <- viewport(x = 0.85, y = 0.55,
                    height = 0.12, width = 0.10,
                    name = "hist")
pushViewport(vp.hist)
# grid.rect(gp = gpar(col="#CCCCCC"))
set.seed(66)
p <- ggplot(data.frame(x = rnorm(100, 33)), aes(x)) + 
  geom_histogram(bins = 7, fill = "steelblue", col = "white") +
  theme_void()
print(p, newpage = FALSE)
...
```{r ddiscretam, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Representación de la función de probabilidad para el experimento de las monedas", out.width="70%"}

ejdiscreta <- data.frame(x = c(0:3),
                         p = c(1/8, 3/8, 3/8, 1/8))
# kable(ejdiscreta, format = "latex", digits = 4)
plot(0:100, type = "n", 
     xlim = c(-0.5, 3.5),
     ylim = c(0, 1), 
     xlab = "x",
     ylab = "p(x)", 
     axes = FALSE)
axis(1, pos = 0, at = c(-5, 0, ejdiscreta$x, 100), col = gray(0.5))
axis(2, pos = -0.5, las = 1, 
     at = c(0, ejdiscreta$p, 1),
     labels = c("", round(ejdiscreta$p, 2), "1"),
     col = gray(0.5))
for (i in seq(along = ejdiscreta$x)){
  lines(rep(ejdiscreta$x[i], 2), 
        c(0, ejdiscreta$p[i]),
        lwd = 3,
        col = "steelblue")
}
...
```{r pdiscretam, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Representación de la función de distribución para el ejemplo de las monedas", out.width="70%"}
ejdiscreta$F <- cumsum(ejdiscreta$p)
# kable(ejdiscreta[, -2], format("latex"), digits = 4)
plot(0:100, type = "n", ylim = c(0, 1), 
     xlim = c(-0.5, 3.5),
     xlab = "x",
     ylab = "F(x)", 
     axes = FALSE)
axis(1, pos = 0, at = c(0, ejdiscreta$x, 100), col = gray(0.5))
axis(2, pos = -0.5, las = 1, 
     at = c(0, ejdiscreta$F, 1),
     labels = c("", round(ejdiscreta$F, 2), "1"),
     col = gray(0.5))
lines(c(-10, ejdiscreta$x[1]),
      rep(0, 2),
      lwd = 2,
      col = "steelblue")
points(ejdiscreta$x[1], 0, pch = 19, cex = 0.8, col = "white")
points(ejdiscreta$x[1], 0, pch = 1, cex = 0.8, col = "steelblue")
lines(c(rev(ejdiscreta$x)[1], 100),
      rep(rev(ejdiscreta$F)[1], 2),
      lwd = 2,
      col = "steelblue")
points(rev(ejdiscreta$x)[1], rev(ejdiscreta$F)[1], pch = 19, cex = 0.8, col = "steelblue")
for (i in 1:(nrow(ejdiscreta) - 1)){
  lines(c(ejdiscreta$x[i], ejdiscreta$x[i+1]),
        rep(ejdiscreta$F[i], 2),
        lwd = 2,
        col = "steelblue")
  points(ejdiscreta$x[i], ejdiscreta$F[i], pch = 19, cex = 0.8, col = "steelblue")
  points(ejdiscreta$x[i+1], ejdiscreta$F[i], pch = 19, cex = 0.8, col = "white")
  points(ejdiscreta$x[i+1], ejdiscreta$F[i], pch = 1, cex = 0.8, col = "steelblue")
}
```
...
```{r ejdiscretat, echo=FALSE}
options(OutDec = ",")
ejdiscreta <- data.frame(x = c(20, 36, 60),
                         p = c(36/52, 12/52, 4/52))
ejdiscreta$F <- cumsum(ejdiscreta$p)
tdata <- ejdiscreta
# colnames(tdata) <- c("$$x_i$$", "$$p(x_i)$$", "$F(x_i)$")
knitr::kable(tdata, digits = 4, col.names = c("$x_i$", "$p(x_i)$", "$F(x_i)$"),
             caption = "Funciones de probabilidad y distribución para la variable discreta del ejemplo ilustrativo", escape = FALSE)
# colnames(tdata) <- c("$x_i$", "$p(x_i)$", "$F(x_i)$")
# pander::pander(tdata, style = "rmarkdown")
options(OutDec = ".")
```
...
```{r ddiscreta, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Representación de la función de probabilidad de una variable aleatoria discreta", out.width="70%"}
...
```{r pdiscreta, echo=FALSE,purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Representación de la función de distribución de la variable aleatoria discreta del ejemplo ilustrativo", out.width="70%"}
...
axis(2, pos = 0, las = 1, 
     at = c(0, ejdiscreta$F, 1),
     labels = c("", round(ejdiscreta$F, 2), "1"), col = gray(0.5))
lines(c(-10, ejdiscreta$x[1]),
      rep(0, 2),
      lwd = 2, col = "steelblue")
points(ejdiscreta$x[1], 0, pch = 19, cex = 0.8, col = "white")
...
```{block, type="rmdcafe"}
Piensa en tu marca de cerveza favorita (o cualquier otra bebida), por ejemplo
en el formato de 33 cl (tercio). Cuando la pides en un bar, 
¿cuál crees que es la probabilidad de que la botella tenga **exactamente**
33 cl?

En realidad, si medimos con una precisión, por ejemplo, de un decimal, podemos obtener
mediciones de $33.0$ _cl_. Pero las mediciones están sujetas a un error, y en realidad
lo que nos está diciendo esa medición es que el volumen está entre $32.95$ y $33.05$,
intervalo del cual sí podemos calcular su probabilidad.
```
...
```{r fdistcambio, echo=FALSE, fig.cap="Función de distribución de una variable continua y probabilidad de un intervalo", fig.align='center', purl=FALSE, fig.width=6, fig.asp=0.6, out.width="70%"}
...
mtext(expression(bold(X)), side = 4, at = 0, las=2)
mtext(expression(bold(F(x))), side = 3, at = 0)
```
...
knitr::include_app("https://elcano.shinyapps.io/derivada/", 
  height = "800px")
```
...
```{r dp, echo=FALSE, fig.cap="Relación entre las funciones de densidad y de probabilidad", purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
...
mtext(expression(bold(f(x))), side = 3, at = 0)
text(3.2, 0.1, expression(F(x)==integral(f(x)*dx, -infinity, x)))
xx <- c(seq(-1, 2, length.out = 100))
polygon(x = c(-1, xx, 2, rev(xx)),
        y = c(0, dnorm(xx, mean = 3), rep(0, 101)), col = gray(0.75))
arrows(2.2, 0.1, 1.8, 0.1, length = 0.1,
       code = 1, lwd=2, col = gray(0.25))
```
...
```{r, ejcontdesc, fig.cap="Función de densidad del ejemplo de los descuentos", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
k <- 1/integrate(function(x) 25-x, 5, 25)$value
densidad <- function(x) {k*(25 - x)}
x <- seq(5,25)
y <- densidad(x)
plot(x, y, type="l",
     axes = FALSE, 
     lwd=2, ylab="f(x)", 
     xlab = "Descuento (%)",
     main ="Función de densidad", xlim=c(0,40))
axis(1, pos = 0)
axis(2, pos= 0 , las = 1)
segments(0, 0, 5.0, lwd = 2)
segments(25, 0 , 40, lwd = 2)
```
...
```{r, ejfdensidad, fig.cap="Representación de la función de densidad del ejemplo", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}

f <- function(x) x/8

curve(f, from = 0, to = 4, 
      xlim = c(-1, 5), axes = FALSE,
      lwd = 2)
axis(1, col = gray(0.5), pos = 0)
axis(2, col = gray(0.5), pos = 0, las = 2)
lines(c(seq(-1, 0)), rep(0, 2), lwd = 2)
lines(c(seq(4, 6, 2)), rep(0, 2), lwd = 2)
...
```{r, ejfdistrib, fig.cap="Representación de la función de distribución del ejemplo", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}

Fdist <- function(x) x^2/16

curve(Fdist, from = 0, to = 4, 
      xlim = c(-1, 5), axes = FALSE,
      lwd = 2,
      ylab = "F(x)")
axis(1, col = gray(0.5), pos = 0)
axis(2, col = gray(0.5), pos = 0, las = 2)
lines(c(seq(-1, 0)), rep(0, 2), lwd = 2)
lines(c(seq(4, 6, 2)), rep(1, 2), lwd = 2)
...
```{r cprocont, echo=FALSE, fig.cap="Cálculo de probabilidades de una variable continua", fig.align='center'}
par(mfrow=c(2,3))
...
```{r densidadfreq, echo=FALSE, fig.cap="Frecuencias, histograma y función de densidad", purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
set.seed(12345)
muestra <- data.frame(volumen = rnorm(500, 33, 2))
library(ggplot2)
ggplot(muestra, aes(volumen)) + 
  geom_histogram(aes(y=..density..), bins = 20, fill = "steelblue", col = "white") +
  stat_function(fun = dnorm, args=list(mean = 33, sd = 2), lwd = 1.5, col = gray(0.25)) +
  labs(title = "Simulación del volumen de 500 botellas de 33 cl", 
       x = "Volumen (cl)",
       y = "Frecuencia relativa/función de densidad")
```
...
```{r, densejilu, fig.cap="Representación de la función de densidad del ejemplo ilustrativo", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
...
axis(2, pos = 0, las = 1)
```
```{r, distejilu, fig.cap="Representación de la función de distribución del ejemplo ilustrativo", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}

marori <- par("mar")
marnew <- marori
marnew[3] <- 2.1
par(mar = marnew)
FF <- function(x) 1 - exp(-2*x)
x <- seq(0.0, 2.5, by =0.05)
y <- sapply(x, FF)
plot(x,y, type = "l", axes = FALSE, lwd = 2,
     ylab = "F(x)", main = "Función de distribución")
axis(1, pos = 0)
axis(2, pos = 0, las = 1)
text(1.5, 0.4, 
     expression(paste(F(x) == integral(2*e^{-2*t}*dt, 0, x)) == 1-e^{-2*x}))
abline(h = 1, col = gray(0.5), lty = 2)
```
...
```{block type='rmdcafe'}
El uso de la palabra esperanza en este ámbito tiene su origen, cómo no, en los
juegos de azar. Así, se hablaba de la esperanza de ganar en el juego (y la
ganancia que se esperaba tener era el resultado), y también del 
_temor_, cuando la esperanza era negativa.
```
...
```{r inversaF, echo=FALSE, fig.cap="La mediana a portir de la inversa de la función de distribución", fig.align='center', purl=FALSE, fig.width=6, fig.asp=0.6, out.width="70%"}
curve(pnorm(x, mean = 3),
      xlim = c(-0.5, 6),
      ylab = "",
      col = "steelblue",
      ...
      axes = FALSE,
      xlab = "")
axis(1, col = gray(0.5), pos = 0, at = c(-10, qnorm(0.5, 3), 10),
     labels = c("", "Me", ""))
axis(2, las = 2, pos = 0, col = gray(0.5),
     at = pnorm(c(-1, 3, 10), mean=3),
     labels = c("", "0.5", ""))
...
```{r, vacentra, fig.cap="Medidas de centralización de una variable aleatoria", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
parw <- c(1.5, 0.4)
f <- function(x) dweibull(x, parw[1], parw[2])
media <- integrate(function(x) x*f(x), lower = -Inf, Inf)$value
mediana <- qweibull(0.5, parw[1], parw[2])
curve(f,lwd = 2, axes = FALSE)
axis(1, pos = 0, col = gray(0.5), padj=1)
axis(2, pos = 0, col = gray(0.5), las = 2)
lines(c(media, media), c(-0.2, 0.2), lwd = 3, col = "steelblue")
xx <- c(seq(0, mediana, length.out = 100))
polygon(x = c(0, xx, mediana, rev(xx)),
        y = c(0, dweibull(xx, parw[1], parw[2]), rep(0, 101)), col = gray(0.75))
lines(c(mediana, mediana), c(-0.2, 0.2), lwd = 3, col = "green4")
yy <- f(xx)
moda <- xx[yy == max(yy)]
lines(c(moda, moda), c(-0.2, 0.2), lwd = 3, col = "red4")
legend(0.7, 1.6, c("Media", "Mediana", "Moda"), lwd = 2, col = c("steelblue", "green4", "red4"))
text(0.2, 1, "F(Me)=0.5")
mtext(c("Mo", "Me", expression(mu)), side = 1, at = c(moda, mediana, media))
...
```{block, type='rmdejemplo'}
Vamos a calcular las medias de las variables aleatorias de los ejemplos de sujetos en estudio. 
...
```{block, type='rmdejemplo'}
Vamos a calcular las varianzas de las variables aleatorias de los ejemplos de sujetos en estudio. 
Para la variable aleatoria discreta: 
...
```{r, cuartil, fig.cap="Cuantiles de una variable aleatoria", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
parw <- c(1.5, 0.4)
f <- function(x) dweibull(x, parw[1], parw[2])
cuartil <- qweibull(0.75, parw[1], parw[2])
curve(f,lwd = 2, axes = FALSE)
axis(1, pos = 0, col = gray(0.5), padj=1)
axis(2, pos = 0, col = gray(0.5), las = 2)
xx <- c(seq(0, cuartil, length.out = 100))
polygon(x = c(0, xx, cuartil, rev(xx)),
        y = c(0, dweibull(xx, parw[1], parw[2]), rep(0, 101)), col = gray(0.75))
text(0.2, 1, expression(italic(F(X[0.75])==0.75)))
mtext(c(expression(italic(X[0.75]))), side = 1, at = c(cuartil))
...
```{r, cheby, fig.cap="Cota superior externa Desigualdad de Chebyshev", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
set.seed(1111)
x <- rnorm(100, sd = 5) + runif(100) + rpois(100, 5)
y <- density(x, bw = 1)
mu <- mean(x)
sig <- sd(x)
k <- 1.5
...
plot(y, type = "l",
     main = "Desigualdad de Chebyshev",
     xlab = "Valores de la variable",
     ylab = "f(x) (desconocida)",
     axes = FALSE, 
     ylim = c(0, 0.12),
     col = gray(0.8), lwd = 2)
axis(1, pos = 0, at = c(-20, li, mu, ls, 30), 
     labels = c("", expression(mu-k*sigma), expression(mu), expression(mu+k*sigma),""))
lines(c(-30, li), c(0,0), col = "red4", lwd = 2)
lines(c(ls, 30), c(0,0), col = "red4", lwd = 2)

x1 <- min(which(y$x >= -30))  
x2 <- max(which(y$x <  li))
with(y, polygon(x=c(x[c(x1,x1:x2,x2)]), 
                y= c(0, jitter(y[x1:x2], factor = 2, amount = 0.008), 0),
                lty = 0,
                col="palegoldenrod"))
...
with(y, polygon(x=c(x[c(x1,x1:x2,x2)]), 
                ...
                lty = 0,
                col="palegoldenrod"))
legend(15, 0.12, expression(p<=frac(1, k^2)), fill = "palegoldenrod", box.lwd = 0)

```

```{r, cheby2, fig.cap="Cota inferior interna Desigualdad de Chebyshev", echo=FALSE, purl=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%"}
set.seed(1111)
x <- rnorm(100, sd = 5) + runif(100) + rpois(100, 5)
y <- density(x, bw = 1)
mu <- mean(x)
...
x2 <- max(which(y$x <  ls))
with(y, polygon(x=c(x[c(x1,x1:x2,x2)]), 
                y= jitter(c(0, y[x1:x2], 0), factor = 2, amount = 0.01), col="palegoldenrod", lty = 0))

legend(15, 0.12, expression(p>= 1-frac(1, k^2)), fill = "palegoldenrod", box.lwd = 0)
