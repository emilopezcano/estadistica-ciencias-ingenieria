...
```{r ejlab, echo=FALSE}
lab %>% slice_head(n=10) |> kable(caption = "Ejemplo datos de laboratorio")
```
...
lab |> count(tipo) |> 
  mutate(f = n/nrow(lab), N = cumsum(n),
         F = cumsum(f)) |> 
  knitr::kable(digits = 3, caption = "Tabla de frecuencias del tipo de queso")
```
...
```{r tarta, fig.align='center', fig.cap="Gráfico de sectores con etiquetas"}
library(ggstatsplot)
