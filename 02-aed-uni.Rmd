...
```{r anscombeuni, echo = FALSE, fig.cap="Representación de las variables del cuarteto de Anscombe", fig.width=8, out.width="100%" }
library(tidyr)
library(ggplot2)
gdata <- 
  anscombe %>% 
...
tdata <- as.data.frame(table(lab$clase)) |>
  mutate(F = cumsum(Freq)/nrow(lab))

if(is_latex_output()){
  tdata |> kable(digits = 3, 
                 col.names = c("\\((L_{i-1}, L_i]\\)",
                               "\\(n_i\\)",
                               F = "\\(F_i\\)"),
                 caption = "Tabla de frecuencias por intervalos")
} else {
  tdata |>  gt(caption = "Tabla de frecuencias por intervalos") |>
...
hist(lab$ph, col = "#CB0017", main = "Histograma pH",
...
lab |> 
  ...
  theme_bw() +
  labs(title = "Histograma recuento de bacterias",
...
lab |> 
  ...
  geom_histogram(bins = 10, fill = "#CB0017", col = "white") +
  theme_bw() +
  labs(title = "Histograma contenido en sal",
     x = "Contenido en sal", 
     y = "Frecuencia")
```
...
```{r histo-resumen, fig.height=7, fig.width = 12, fig.align='center', out.width="90%", echo=FALSE, fig.cap="Histogramas de variables con distintos tipos de curtosis"}

library(gridExtra)
grid.arrange(
  ggplot(lab, aes(x = ph)) + 
    geom_histogram(bins = 10, fill = "orange", col = "white") + 
    theme_bw(),
  ggplot(lab, aes(x = est)) + 
