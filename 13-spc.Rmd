...
[Apéndice](https://lcano.com/b/epac/_book/ap-tablas.html#constantes-de-shewhart)
...
https://lcano.com/b/epac/datos/ejemplos_espesor2.xlsx
```
...
# Gráficos de control con R

- Paquete `qcc`

- Requiere cada grupo en una fila de una tabla

- Los datos se preparan con la función `qccGroups`
...
'```{r thickness, echo=FALSE}
library(SixSigma); library(tidyr); library(dplyr)
ss.data.thickness2 |> 
  select(-flaws, - ushift) |> 
  mutate(muestra = paste0("m", rep(1:6, times = 14))) |> 
  spread(key = muestra, value = thickness) |> 
knitr::kable(caption = "Datos del espesor de placas metálicas")
'```
...
# Ejemplo: espesor placas de acero

'```{r, fig.height=4, fig.align="center", fig.show="hide"}
library(SixSigma)
library(qcc, quietly = TRUE)
samples.thick <- qccGroups(
  data = ss.data.thickness2,
...
'```{r, fig.height=4, fig.align="center", out.width="80%", echo=FALSE}
library(SixSigma)
library(qcc, quietly = TRUE)
samples.thick <- qccGroups(
  data = ss.data.thickness2,
...
# Ejemplo: espesor placas de acero (excel)
...
# Ejemplo gráfico del rango Minitab

- Los datos pueden estar dispuestos apilados o extendidos

- Importar a hoja de datos (puede ser directamente de Excel)

- Estadísticas > Gráficas de control > Gráficas de variables para subgrupos > Xbarra

- Indicar las columnas que tienen los datos, o el tamaño de los subgrupos, o la 
columna identifidadora.
```
...
Valores **pre-especificados**, $LC = d_2 \sigma_0$,
$$LCI = d_2\sigma_0 - 3 d_3 \frac{d_2 \sigma_0}{d_2} = \sigma_0 (d_2 - 3d_3) = D_1 \sigma_0$$
$$LCS = d_2\sigma_0 + 3 d_3 \frac{d_2 \sigma_0}{d_2} = \sigma_0 (d_2 + 3d_3) = D_2 \sigma_0$$

Valores **estimados**: $LC = \overline{R}$
$$LCI = \overline{R} - 3 d_3 \frac{\overline{R}}{d_2} = \overline{R} \left (1 - \frac{3d_3}{d_2}\right ) = D_3 \overline{R}$$
$$LCS = \overline{R} + 3 d_3 \frac{\overline{R}}{d_2} = \overline{R} \left (1 + \frac{3d_3}{d_2}\right ) = D_4 \overline{R}$$
...
a ellas. En **Minitab**, seleccionaríamos en el menú el gráfico R.
```


```{r grango2, fig.height=4, fig.align='center', fig.cap="Gráfico del rango para el ejemplo del espesor"}
...
1. Primero comprobar variabilidad

2. Solo si está controlada, comprobar las medias

3. Habitualmente se presentan juntos
...
# Gráfico de control de la media (con desviación típica)

**Valores pre-especificados**

- Igual que el del rango
...
**Valores pre-especificados**: $LC = c_4\sigma_0$
$$LCI = c_4\sigma_0 - 3 c_4\sigma_0\frac{\sqrt{1-c_4^2}}{c_4}  = \sigma_0 \left(c_4 - 3\sqrt{1-c_4^2} \right) = B_5\sigma_0$$
$$LCS = c_4\sigma_0 + 3 c_4\sigma_0\frac{\sqrt{1-c_4^2}}{c_4}  = \sigma_0 \left(c_4 + 3\sqrt{1-c_4^2} \right) = B_6\sigma_0$$


**Valores estimados**: $LC = \overline{s}$
$$LCI = \overline{s} - 3 \overline{s}\frac{\sqrt{1-c_4^2}}{c_4}  = \overline{s} \left(1 - \frac{3\sqrt{1-c_4^2}}{c_4} \right) = B_3\overline{s}$$
$$LCS = \overline{s} + 3 \overline{s}\frac{\sqrt{1-c_4^2}}{c_4}  = \overline{s} \left(1 + \frac{3\sqrt{1-c_4^2}}{c_4} \right) = B_4\overline{s}$$
...
```{block2, type='rmdpractica'}
Vamos a completar el ejemplo anterior ilustrando las fases I y II. Hagamos dos
...
```{r gms, fig.cap="Gráfico de la media y la desviación típica monitorizando nuevos datos", message=FALSE, warning=FALSE}
...
# Ejemplo: gráfico $\bar{x}$ - $S$ fase II

'```{r, fig.show='hide'}
new.sample <- matrix(c(0.719, 0.759, 0.708, 0.83, 0.766, 0.709), ncol = 6)
ccxbar <- qcc(data = samples.thick, type = "xbar",
              center = 0.76,  std.dev = 0.025,
              newdata = new.sample, newlabels = "8.1")
...
- Cuando no es posible hacer grupos racionales

- La variabilidad se controla por la variación cada dos observaciones
...
# Diseño del gráfico I
  
**Con valores pre-especificados**: $LC=\mu_0$
$$LCI=\mu_0 - 3\sigma_0$$
$$LCS=\mu_0 + 3\sigma_0$$
  
**Sin valores pre-especificados**: $LC=\overline{X}$
...
# Diseño del gráfico MR
  
**Con valores pre-especificados**: $LC = d_2 \sigma_0 = 1.128 \sigma_0$
$$LCI = d_2\sigma_0 - 3 d_3 \frac{d_2 \sigma_0}{d_2} = \sigma_0 (d_2 - 3d_3) \leadsto 0$$
$$LCS = d_2\sigma_0 + 3 d_3 \frac{d_2 \sigma_0}{d_2} = \sigma_0 (d_2 + 3d_3) = D_2 \sigma_0 = 3.686\sigma_0$$
  
**Sin valores pre-especificados**: $LC = \overline{R}_m$
$$LCI = \overline{R}_m - 3 d_3 \frac{\overline{R}_m}{d_2} = \overline{R}_m \left (1 - \frac{3d_3}{d_2}\right ) \leadsto 0$$
$$LCS = \overline{R}_m + 3 d_3 \frac{\overline{R}_m}{d_2} = \overline{R}_m \left (1 + \frac{3d_3}{d_2}\right ) = D_4 \overline{R}_m = 3.267 \overline{R}_m$$
```

```{block2, type='rmdpractica'}
A modo ilustrativo, tomamos las primeras 24 observaciones de los datos 
...
```{r gimm, fig.cap="Gráficos de valores individuales y media móvil"}
...
# Ejemplo gráficos de valores individuales y rangos móviles
...
# Tamaño de subgrupos y frecuencia de muestreo

- Muestras para estimar población

- Se cometen dos tipos de errores:
    - Tipo I: El proceso está bajo control y la muestra cae fuera
    - Tipo II: El proceso está fuera de control y la muestra cae dentro

- Probabilidad tipo I $\alpha$ está fijado en 0.0027 ($\pm3\sigma$)
- Probabilidad tipo II $\beta$ se puede calcular en función de $n$ y $\delta$
  (número de desviaciones típicas que queremos detectar como cambio).
- O bien encontrar un tamaño $n$ para un determinado $\beta$ y número de
$k$ desviaciones típicas a detectar.
...
```{r despmedia, echo=FALSE,fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="Efecto del desplazamiento de la media en el gráfico de control", out.width="90%"}
set.seed(6666)
plot(rnorm(15), ylim = c(-5,5), xlim = c(0, 26), pch = 16, col = "steelblue",
    main = "Efecto del desplazamiento de la media",
    ylab = "CTQ", xlab = "Pieza")
abline(h = c(-3,3), col = "steelblue")
points(16:25, rnorm(10, 2), pch = 16, col = "tomato4")
abline(h = c(-3,3), col = "steelblue")
lines(c(0,15.5), c(0,0), col = "steelblue", lwd = 2)
lines(c(15.5,25.5), c(2,2), col = "tomato4", lwd = 2)
abline(h = c(-1,5), col = "tomato4")
```
...
```{r occ, echo=FALSE,fig.width=6, fig.asp=0.6, fig.align='center', fig.cap="curva de operación característica para el gráfico de la media y varios valores de $n$", out.width="70%", message=FALSE, warning=FALSE}
library(qcc, quietly = TRUE)
data(pistonrings)
diameter <-
  qccGroups(data = pistonrings,
            x = diameter,
            sample = sample)
objqcc <- qcc(diameter,
              type="xbar",
...
# Tamaño medio de racha (ARL)

- De forma natural, alguna vez tendremos una falsa alarma

- Para un proceso bajo control:
  
$$\mathit{ARL}=\frac{1}{p}= \frac{1}{0.0027}=370.37.$$
  
- Para un proceso que se desvíe:$\mathit{ARL}=\frac{1}{1-\beta}

Por ejemplo, para $k=1$ y $n=5$, $\beta=0.25$. Entonces:

$$\mathit{ARL} = \frac{1}{1-0.25}=4$$

muestras en promedio para detectar el cambio.  
...
- Controlamos recuentos o proporciones

- En grupos de tamaño $n$
  
- Distribución binomial

- Función `qcc`: Añadir tamaños de grupo (`sizes`)
...
**Con valores pre-especificados**: $p_j = \frac{D_j}{n_j}$
$$LC = p_0.$$
$$LCI = p_0 - 3 \sqrt{p_0(1-p_0)/n}$$
$$LCS = p_0 + 3 \sqrt{p_0(1-p_0)/n}$$
  
**Sin valores pre-especificados**: 
  
Estimar $p$ y aplicar la fórmula 
...
# Diseño del gráfico $np$
  
**Sin valores pre-especificados**:  $D_j$
$$LC = n\bar{p}$$
$$LCS = n\bar{p} + 3 \sqrt{n\bar{p}(1-\bar{p})}$$
$$LCI = n\bar{p} - 3 \sqrt{n\bar{p}(1-\bar{p})}$$
...
```{r gp, fig.cap="Gráfico p de proporción piezas espesor mayor de 0.775"}
thick.attribute <- aggregate(thickness ~ ushift, 
    data = ss.data.thickness2,
    FUN = function(x){
      sum(x>0.775)
...
- Por unidad (tiempo, superficie, ...)
  
- Distribución de Poisson

- Función `qcc`: Para el gráfico c, no es necesario tamaño.
...
```{block2, type='rmdpractica'}
En el conjunto de datos que venimos utilizando, se inspeccionan aleatoriamente
...
```{r graficoc, fig.cap="Gráfico c de los defectos"}
...
```{block2, type='rmdpractica'}
El siguiente código de **R** produce el gráfico de la figura \@ref(fig:graficou).
...
```{r graficou, fig.cap="Gráfico u de los defectos por unidad"}
...
# Diseño del gráfico $c$
  
**Con valores pre-especificados**: Eventos $c_j$
$$LC = c_0$$
$$LCI = c_0 - 3 \sqrt{c_0}$$
$$LCS = c_0 + 3 \sqrt{c_0}$$
...
Estimar $c$ y aplicar la fórmula 
$$\overline{c} = \frac{\sum\limits_{j=1}^{m}{c_j}}{m}$$
```
...
# Diseño del gráfico $u$
  
**Sin valores pre-especificados**: número medio por unidad $u_j = \frac{c_j}{n_j}$
$$LC = \bar{u} = \frac{\sum\limits_{j=1}^m u_j}{\sum\limits_{j=1}^mn_j}$$
$$LCI_i = \bar{u} - 3 \sqrt{\frac{\bar{u}}{n_j}}$$
$$LCS_i = \bar{u} + 3 \sqrt{\frac{\bar{u}}{n_j}}$$

**Con valores pre-especificados**: 
  
sustituir $\overline{u}$ por $u_0$
```
...
```{r tiposg, echo=FALSE, fig.cap="Selección del gráfico de control", out.width="100%"}
knitr::include_graphics("images/seleccion.png")
```
...
digraph seleccion {
  ...
  node [shape = box]
    ...
    b [label = 'Variable']
    c [label = 'Atributo']
    e [label = 'Gráfico I-RM']
    f [label = 'Gráfico Media-R']
    g [label = 'Gráfico Media-S']
    h [label = 'Defectuosos en n']
    i [label = 'Defectos por unidad']
    j [label = 'Gráfico p']
    k [label = 'Gráfico np']
    l [label = 'Gráfico c']
    m [label = 'Gráfico u']
  ...
  c -> h
  c -> i
  h -> k [label = 'n constante']
  h -> j [label = 'mide fracción']
  i -> l [label = 'recuento']
  i -> m [label = 'media']
}
")
...
```{r estrategia, echo=FALSE, fig.cap="Estrategia SPC", out.width="100%"}
knitr::include_graphics("images/estrategia.png")
```
...
digraph estrategia {
  ...
  node [shape = box]
    a [label = 'Elegir CTQ']; 
    b [label = 'Elegir n, frecuencia y k (n.grupos)']
    d [label = 'Análsis exploratorio']
    e [label = 'Cálculos gráficos\nMedia, R, S, Límites']
    j [label = 'Calcular rendimiento (Ppk)']
    m [label = 'Reducir inspección']
  node [shape = egg]
    c [label = 'Plan medición.\nRecogida y control']
    g [label = 'Investigar y eliminar\n causas asignables']
    i [label = 'Investigar y eliminar\n causas asignables']
    l [label = 'Inspección 100%']
  node [shape = diamond]
    f [label = 'Gráfico R (o S)']
    h [label = 'Gráfico Media']
    k [label = 'Calcular capacidad (Cpk)']
    
  a -> b -> c -> d -> e -> f
  e -> j
  f -> h [label = 'En Control']
  f -> g [label = 'Fuera Control']
  g -> f
  h -> i [label = 'Fuera Control']
  i -> h
  h -> k
  k -> l [label = 'No Capaz']
  l -> k [label = 'Mejorar']
...
Realiza gráficos de control con los datos de tu proceso y con los datos de ejemplo
...
3. La voz del proceso

4. Capacidad y rendimiento
...
# La voz del cliente (_VoC_)

- Especificaciones de requisitos, $Y$

- Especificaciones de variables críticas, $X$

- Objetivo (T) + tolerancia

- Tipos de especificación:
    - Valor ideal: $L < X < U$
    - Cuanto más bajo mejor: $X < U$
    - Cuanto más alto mejor: $X > L$
???
...
```{r voc, echo=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%", fig.cap="Límites de especificación de una característica (VoC)"}
curve(dnorm(x), -5, 5, type="l",
    lty=2, ylab="frecuencia", 
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/voc-1.png)
...
```{r his, echo=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%", fig.cap="La voz del proceso (VoP)"}
hist(merma$merma1, freq = FALSE, 
     main = "Histograma de la variable merma",
     xlab = "Merma (gr.)",
     ylab = "",
     las = 1)
lines(density(merma$merma1), lwd=2)
box("outer")
```
```{r taguchi2, echo=FALSE, fig.width=8, fig.asp=0.6, fig.align='center', fig.cap="Función de pérdida de Taguchi", out.width="70%"}
# par(bg="white")
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/his-1.png)
background-position: 50% 70%
background-size: 80%  
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/taguchi2-1.png)
background-position: 50% 70%
background-size: 80%  
...
--
$$\hat{\sigma}_{LT}=s=\sqrt{\frac{\sum (x_i - \overline{x})^2}{n-1}}.$$
  
$$\hat{\sigma}_{ST}=\sqrt\frac{\sum s_i^2 }{m} \qquad \text{o bien} \quad \frac{\overline{R}}{d_2}, \frac{\overline{s}}{c_4}$$
```

```{r, echo=FALSE}
suma <- paste0(paste0(round(sapply(varlongterml, function(x) sd(x$valor)), 2), "^2"), collapse = " + ")
vst <- round(sum(round(sapply(varlongterml, function(x) sd(x$valor)), 2))/length(varlongterml), 2)
```

```{block2, type='rmdejemplo'}
En la figura \@ref(fig:nubelt) del capítulo \@ref(ch:intro) se mostraba un ejemplo
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/nubelt-1.png)
background-position: 50% 90%
background-size: 70%


# Variación a largo plazo y a corto plazo
'```{r, echo=FALSE}
suma <- paste0(paste0(round(sapply(varlongterml, function(x) sd(x$valor)), 2), "^2"), collapse = " + ")
vst <- round(sum(round(sapply(varlongterml, function(x) sd(x$valor)), 2))/length(varlongterml), 2)
'```
...
$$\sigma_{ST} = \sqrt\frac{`r suma`}{`r length(varlongterml)`}=`r vst`$$
...
.centro[.imgwpeque2[![image](../img/idea.png)]]
¿Cuántas veces cabe mi proceso dentro de la especificación?

--

.centro[.imgwpeque2[![image](../img/idea.png)]]
Las fórmulas generales se basan en cuantiles en vez de $\mu$ y $\sigma$ (solo para normalidad)

.centro[.imgwpeque2[![image](../img/idea.png)]]
Se pueden calcular con valores preespecificados $\mu_0$ y $\sigma_0$
...
de defectos es del `r formatC(2*pnorm(-3)*100,2 )`%.
...
```{r natlim, echo=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%", fig.cap="Límites de referencia del proceso"}
curve(dnorm(x), -6,6, type="l",lty=2, ylab="Frecuencia", 
    xlab="Número de desviaciones típicas",
    axes=FALSE,
    ylim=c(0,.6))
axis(side=1, labels=TRUE, pos = 0 )
abline(v=-3)
abline(v=3)
text(-3,.6,expression(mu*-3*sigma),pos=4)
text(3,.6,expression(mu*+3*sigma),pos=2)
arrows(c(-3,3),c(.5,.5),c(3,-3),c(.5,.5))
text(0, .45, "99.73% (Límites de tolerancia naturales)", pos = 1)
box("outer")
```
...
```{r voct, echo=FALSE, fig.width=6, fig.asp=0.6, fig.align='center', out.width="70%", fig.cap="Objetivo de la característica no centrado en el punto central de la especificación"}
curve(dnorm(x), -5, 5, type="l",
    lty=2, ylab="frecuencia", 
    xlab="Valor observado de la característica",
    axes=FALSE)
...
- Para objetivo no centrado en la especificación
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/voct-1.png)
background-position: 50% 70%
background-size: 80%  

# Objetivo no centrado
...
Se calculan todos los que sean apropiados:
...
- Compara índices corregidos con índices sin corregir. Cuanto más se parezcan,
más centrado estará nuestro proceso en la especificación, lo que siempre es
deseable.
...
```{r indices, echo=FALSE, fig.width=6, fig.asp=0.4, fig.align='center', out.width="100%", fig.cap="Interpretación de los índices de capacidad"}
par(mfrow=c(1,3))
curve(dnorm(x), -5, 5, type="l",
    lty=2, ylab="frecuencia", 
    xlab=expression(U-L==6*sigma),
    axes=FALSE,
    main = expression(C[pk]==1))
abline(v=-3, lwd = 2, col = "darkred")
abline(v=3, lwd = 2, col = "darkred")
abline(h=0)
text(-4.15,0.35,"L=LCI")
text(4.15,0.35,"U=LCS")
mtext(expression(mu),1)
...
mtext(expression(mu),1)
## Especificación
abline(v=-4, lwd = 2, col = "darkred")
text(-4.40,0.35,"L")
abline(v=4, lwd = 2, col = "darkred")
text(4.40,0.35,"U")

## Control
abline(v=-3, lwd = 2, col = "deepskyblue")
text(-3.40,0.4,"LCI")
abline(v=3, lwd = 2, col = "deepskyblue")
text(3.40,0.4,"LCS")
...
mtext(expression(mu),1)
## Especificación
abline(v=-2.5, lwd = 2, col = "darkred")
text(-2,0.35,"L")
abline(v=2.5, lwd = 2, col = "darkred")
text(2,0.35,"U")
...
class: large
background-image: url(../_book/estadistica-aplicada_files/figure-html/indices-1.png)
...
--
.centro[.imgwpeque2[![image](../img/idea.png)]]
Los índices calculados son .red[estimaciones] sujetas a error. Antes de tomar decisiones,
comprobar el intervalo de confianza.
```
...
```{r capexplo, out.width="90%", fig.align="center", fig.height=4, fig.cap="Variabilidad a corto y a largo plazo"}
library(lattice)
...
```{r capespesor, fig.cap="Estudio de capacidad para los datos del espesor"}
library(qcc)
...
https://support.minitab.com/es-mx/minitab/18/help-and-how-to/quality-and-process-improvement/capability-analysis/how-to/capability-analysis/normal-capability-analysis/before-you-start/example/
```

```{block2, type='slide', echo=FALSE}
---
# Ejemplo estudio de capacidad

'```{r, out.width="90%", fig.align="center", fig.height=4}
library(lattice)
dotplot(thickness ~ shift | day, 
    data = ss.data.thickness2,
    layout = c(7, 1))	
...
```{block2, type='slide', echo=FALSE, results="hide"}
---
# Ejemplo estudio de capacidad

'```{r, out.width="60%", echo=FALSE, fig.align="center", results="hide"}
plot(cap)
'```
```
...
```{block2, type='slide', echo=FALSE}
---
# Ejemplo estudio de capacidad (SixSigma)
...
```{r capss, fig.cap="Estudio de capacidad para los datos de una bodega"}
library(SixSigma)
ss.study.ca(ss.data.ca$Volume, rnorm(40, 753, 3), 
        LSL = 740, USL = 760, T = 750, alpha = 0.05, 
             f.sub = "Winery Project")
...
```{block2, type='hide', echo=FALSE}
Finalmente, véase la relación del análisis de capacidad con los gráficos
de control y la inspección por muestreo, reflejadas en las figuras
\@ref(fig:estrategia) y \@ref(fig:metodos) de los capítulos \@ref(ch:gc) y
\@ref(ch:aceptacion) respectivamente.
```
...
Evalúa las condiciones de capacidad y rendimiento de tus procesos, calcula índices
y realiza un estudio completo.
